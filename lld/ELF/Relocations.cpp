...
static void addTgotEntry(Symbol &sym) {
  ...
  if (!sym.isPreemptible && config->isCheriAbi) {
    invokeELFT(in.tgotCapRelocs->addCapReloc,
               {in.tgot.get(), sym.getTgotOffset()}, {&sym, 0},
               sym.isPreemptible, 0);
    return;
  }
  in.relaTgot->addReloc({target->tgotRel, in.tgot.get(), sym.getTgotOffset(),
                         sym.isPreemptible || config->isCheriAbi
...
static unsigned handleTlsRelocation(RelType type, Symbol &sym,
                                    InputSectionBase &c, uint64_t offset,
                                    int64_t addend, RelExpr expr) {
  bool isTgot = oneof<R_TGOT, R_TGOT_TP, R_TGOT_GOT, R_TGOT_GOT_PC,
                      R_TGOT_TLSDESC, R_TGOT_TLSDESC_CALL,
  ...
  if (oneof<R_AARCH64_TLSDESC_PAGE, R_TLSDESC, R_TLSDESC_CALL, R_TLSDESC_PC,
    ...
    if (sym.isPreemptible && !isTgot) {
      ...
      c.addReloc({target->adjustTlsExpr(type, relaxExpr), type,
                  offset, addend, &sym});
    } else {
      RelExpr relaxExpr;
      if (isTgot)
